...
from ut_version import get_last_commit_timestamp

logger = logging.getLogger(__name__)

MAX_CLUSTERS_LIMIT = 10
...
def _kmeans_1_prepare_data(
    df_uploaded: pd.DataFrame,
    produce_output_to_ui: bool = True,
) -> tuple:
    ...
    X_scaled_2d_array: np.ndarray = scaler.fit_transform(X)
    X_scaled_df: pd.DataFrame = pd.DataFrame(X_scaled_2d_array, columns=numeric_cols)
    if produce_output_to_ui:
        st.dataframe(X_scaled_df.head())

    return X_scaled_df, numeric_cols, df, X_scaled_2d_array


def _kmeans_2_find_optimal_k(X_scaled: np.ndarray, n_samples: int) -> tuple[int, int]:
    ...
    K_MAX = min(MAX_CLUSTERS_LIMIT, n_samples // 20)
    sse: dict[int, float] = {}
    ...
    if len(sse) >= 3:
        ...
        # idxmin
        #   3
        # find 5 (optimal_k_auto) by 3 (idxmin)
...
def _override_k_by_user_supplied_value(
    optimal_k_auto: int,
    K_MAX: int,
) -> int:
...
def _kmeans_3_run_kmeans_and_evaluate_silhouette_score(
    X_scaled: np.ndarray,
    df: pd.DataFrame,
    recommended_k: int,
) -> pd.DataFrame:
...
def _kmeans_4_label_and_summarize_clusters(
    df: pd.DataFrame,
    recommended_k: int,
    label_criterion: str,
    ...
    # 1. Sort the clusters by user supplied field in descending order
...
def _kmeans_5_visualize_clusters(
    df: pd.DataFrame,
    label_criterion: str,
...
def _kmeans_6_profile_clusters(
    alphabet_labels: list,
    ...
    def _profile_cluster(
        ...
        # Start profile text
        profile = [
            f"**Cluster {cluster_label}** (Samples: {int(cluster_row['Count']):,} | {cluster_row['Count (%)']}%):"
        ]
    ...
    st.subheader("6. Natural Language Cluster Profiling")
    ...
    for label in alphabet_labels:
        ...
        profile_text = _profile_cluster(label, cluster_summary, label_criterion, numeric_cols)
...
def _explore_tsne_perplexity(
    ...
    """
    st.header("EDA - t-SNE Perplexity Exploration")
    ...
    perplexity_list: list[int] = [5, 10, 20, 30, 40, 50, 100]
...
def run_kmeans_analysis(
    ...
    # Load and z-scale data
    prep_results: tuple = _kmeans_1_prepare_data(df_uploaded)
    if prep_results is None:
        return
    # X_scaled_df: pd.DataFrame
    numeric_cols: list[str]
    df: pd.DataFrame
    X_scaled_2d_array: np.ndarray
    _, numeric_cols, df, X_scaled_2d_array = prep_results  # type: ignore

    # Find optimal k
    optimal_k_auto: int
    K_MAX: int
    optimal_k_auto, K_MAX = _kmeans_2_find_optimal_k(X_scaled_2d_array, len(df))

    # Override k if user supplied a new value
    recommended_k: int = _override_k_by_user_supplied_value(optimal_k_auto, K_MAX)

    # Run K-Means and find Silhouette score
    df = _kmeans_3_run_kmeans_and_evaluate_silhouette_score(X_scaled_2d_array, df, recommended_k)

    # Cluster Labeling and Summary (Centroids)
    df, cluster_summary, alphabet_labels = _kmeans_4_label_and_summarize_clusters(df, recommended_k, label_criterion)

    # Visualization
    _kmeans_5_visualize_clusters(df, label_criterion, alphabet_labels)

    # Natural Language Profiling
    _kmeans_6_profile_clusters(alphabet_labels, cluster_summary, label_criterion, numeric_cols)


# --- Streamlit UI Main Function ---
def main():
    last_updated: str = get_last_commit_timestamp()
    st.title("Segmenting Data using ML K-Means")
    st.markdown(
    ...
    if df is not None:
        ...
        st.subheader("Run t-SNE or K-Means")
        col_tsne, col_kmeans = st.columns(2)
        ...
        with col_tsne:
            if st.button("Run t-SNE (EDA)"):
                ...
                st.info("Running t-SNE visualization (Exploratory Data Analysis)...")
                _explore_tsne_perplexity(X_scaled_df)
